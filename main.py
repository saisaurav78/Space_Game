...
def load_image(path, scale=None):
    try:
        ...
        surface.fill((255, 0, 255))  # Magenta as error indicator
        return surface
...
def load_assets():
    assets = {
        ...
        'bullet_sound': load_sound("assets/sounds/bullet.mp3"),
        'menu_select_sound': load_sound("assets/sounds/menu_select.mp3"),
        'explosion_sound': load_sound("assets/sounds/explosion.mp3"),
    }
...
class Menu:
        ...
        def render(self, screen):
            ...
            title_text = title_font.render("Welcome to Space World", True, (255, 255, 255))
...
# Function to display an explosion animation at (x, y), scaling over time for a set duration
def show_explosion(x, y, duration=190):
    ...
    while pygame.time.get_ticks() - start_time < duration:
        ...
        rect = scaled_blast.get_rect(center=(x, y))
...
def spawn_enemies(count):
    return [Enemy(assets['enemy'], randint(50, SCREEN_WIDTH-100), randint(50, 100), 1 if randint(0, 1) else -1) for _ in range(count)]
    ...
    if game_state == GameState.PLAYING:
            ...
            for bullet in player_bullets[:]:
                if math.sqrt((enemy.x - bullet.x)**2 + (enemy.y - bullet.y)**2) < 30:
                    ...
                    if enemy.health <= 0:
                        ...
                        score += 10
                        show_explosion(enemy.x + enemy.width//2, enemy.y + enemy.height//2)
                    break
        ...
        for bullet in enemy_bullets[:]:
            if (player.health > 0 and math.sqrt((player.x - bullet.x)**2 + (player.y - bullet.y)**2) < 30):
                ...
                player.health -= 10
                if player.health <= 0:
                    show_explosion(player.x + player.width//2, player.y + player.height//2)
